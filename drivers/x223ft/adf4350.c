...
static int adf4350_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd){
		case CMD_ADF4350_A_SET:
		case CMD_ADF4350_A_GET_LD:
			base_addr = ADF4350_A_BASE;
			break;
		case CMD_ADF4350_B_SET:
		case CMD_ADF4350_B_GET_LD:
			base_addr = ADF4350_B_BASE;
			break;
		default:
			up(&adf4350_st.sem);
			return -ENOTTY;
...
static int __init adf4350_init(void)
{
	...
	if (ret)
		...
		printk("BSP: x223ft ADF4350 Driver installed\n");
...
static void __exit adf4350_exit(void)
{
	...
	printk("BSP: X223FT ADF4350 Driver removed\n");
}
...
MODULE_DESCRIPTION("x223FT ADF4350");
MODULE_LICENSE("GPL");
