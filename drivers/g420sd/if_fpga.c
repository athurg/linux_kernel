	...
	switch(elem.type){
		...
		case normal:
			...
			for (i=0; i<elem.wlen; i++){
				...
				//printk("addr:data = %04X:%04X\n",elem.addr,if_fpga_st.kbuf[i]);
				if (elem.type == normal)
					elem.addr += 2;
			}
			break;
...
static int __init if_fpga_init(void)
{
	...
	printk("BSP: g420sd IF_FPGA Driver installed!\n");
	return 0;
...
fail_registe:
	kfree(kbuf);
fail_malloc:
	printk("BSP: Fail to install g420sd IF_FPGA driver\n");
	return ret;
}
...
static void __exit if_fpga_exit(void)
{
	...
	printk("BSP: g420sd IF_FPGA Driver removed\n");
}
...
MODULE_DESCRIPTION("g420sd IF_FPGA");
MODULE_LICENSE("GPL");
