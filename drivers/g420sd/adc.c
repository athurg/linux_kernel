...
static ssize_t adc_write(struct file *filp, const char __user *buf, size_t size, loff_t *ppos)
{
	...
	if(elem.dev == DEV_ADC_A)
		base = 	ADCA_BASE;
	else{
		printk("BSP: %s elem.dev  must be equal to DEV_ADC_A\n", __FUNCTION__);
		up(&adc_st.sem);
		return 	EFAULT;
	}
...
static int __init adc_init(void)
{
	...
	if (ret)
		...
		printk("BSP: G420SD RX_ADC Driver installed\n");
...
static void __exit adc_exit(void)
{
	...
	printk("BSP: G420SD RX_ADC Driver removed\n");
}
...
MODULE_DESCRIPTION("G420SD RX_ADC");
MODULE_LICENSE("GPL");
