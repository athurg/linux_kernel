...
static int adf4350_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd){
		case CMD_ADF4350_TX_SET:
		case CMD_ADF4350_TX_GET_LD:
			base_addr = ADF4350_TX_BASE;
			break;
		case CMD_ADF4350_RXFB_SET:
		case CMD_ADF4350_RXFB_GET_LD:
			base_addr = ADF4350_RXFB_BASE;
			break;
		default:
			up(&adf4350_st.sem);
			return -ENOTTY;
...
static int __init adf4350_init(void)
{
	...
	adf4350_st.dev.name = "nts_adf4350";
	...
	if (ret)
		...
		printk("BSP: G420SD ADF4350 Driver installed\n");
...
static void __exit adf4350_exit(void)
{
	...
	printk("BSP: G420SD ADF4350 Driver removed\n");
}
...
MODULE_DESCRIPTION("G420SD ADF4350");
MODULE_LICENSE("GPL");
