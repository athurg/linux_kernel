...
static int reset_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if(cmd != CMD_RESET){
		...
		if (arg == RESET_WDG) {
			__raw_writeb((arg & 0xFF), WATCHDOG_BASE);
		} else {
...
static int __init reset_init(void)
{
	...
	if (ret)
		...
		printk("BSP: g420sd RESET Driver installed\n");
...
static void __exit reset_exit(void)
{
	...
	printk("BSP: g420sd RESET Driver removed\n");
}
...
MODULE_DESCRIPTION("g420sd RESET");
MODULE_LICENSE("GPL");
