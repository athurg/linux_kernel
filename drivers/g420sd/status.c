...
static int __init status_init(void)
{
	...
	if (ret) {
		...
		printk("BSP: g420sd STATUS Driver installed\n");
		ret = __raw_readb(DETECT_BASE);
		printk("BSP:	PA board is %s connect\n", DETECT_PA(ret) ? "not":"");
		printk("BSP:	 %s status\n", DETECT_AGE(ret) ? "aged":"not aged");
		ret = 0;
	}
	return ret;
}
...
static void __exit status_exit(void)
{
	...
	printk("BSP: g420sd STATUS Driver removed\n");
}
...
MODULE_DESCRIPTION("g420sd STATUS and DETECT");
MODULE_LICENSE("GPL");
