...
static void lpc3xxx_pcm_dma_irq(int channel, int cause,
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		lpc32xx_dma_queue_llist_entry(prtd->dmach, (void *) prtd->period_ptr,
			...
			(void *) I2S_TX_FIFO(I2S1_BASE),
#else
			(void *) I2S_TX_FIFO(I2S0_BASE),
#endif
			prtd->period_size);
	}
	else {
...
#if defined(CONFIG_SND_LPC32XX_USEI2S1)
			(void *) I2S_RX_FIFO(I2S1_BASE),
#else
			(void *) I2S_RX_FIFO(I2S0_BASE),
#endif
...
static int lpc3xxx_pcm_suspend(struct platform_device *pdev,
			       struct snd_soc_dai *dai)
{
...
static int lpc3xxx_pcm_resume(struct platform_device *pdev,
			      struct snd_soc_dai *dai)
{
	struct snd_pcm_runtime *runtime = dai->runtime;
	struct lpc3xxx_dma_data *prtd;
